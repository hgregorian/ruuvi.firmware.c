...
#define MANAGEMENT_IDLE_TIMEOUT_MS (15U * 1000U)
...
static void management_timeout_isr (void * const p_context)
{
    ...
    const rd_status_t err_code =
        ri_scheduler_event_put (NULL, 0U, &management_disable);
...
static rd_status_t dis_init (ri_comm_dis_init_t * const p_dis, const bool secure)
{
    ...
    name_idx = snprintf (
        p_dis->fw_version,
        sizeof (p_dis->fw_version),
        "%s",
        APP_FW_NAME
    );

    name_idx += snprintf (
        p_dis->fw_version + name_idx,
    ...
    snprintf (
        p_dis->manufacturer,
        sizeof (p_dis->manufacturer),
        "%s",
        RB_MANUFACTURER_STRING
    );
...
rd_status_t app_comms_init (const bool secure)
{
    ...
    err_code |= ri_timer_create (&m_management_timer, RI_TIMER_MODE_SINGLE_SHOT,
                                 &management_timeout_isr);
